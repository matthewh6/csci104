...
std::set<std::string> makeRandomAlphaStringSet(size_t count, RandomSeed seed, size_t stringLength)
{
	// set up random number generator
	std::mt19937 randEngine;
	randEngine.seed(seed);
	...
	do
	{
		...
		if(randomSet.find(currString) != randomSet.end())
		{
			//duplicate
			continue;
		}

		randomSet.insert(currString);
